...
plt.legend()
plt.grid()
plt.title("Offset: 500mV")
plt.savefig('im2/exercicioD_Tensoes500mV')
...
plt.savefig('im2/exercicioD_capacitancia500mV')
'''
#Calcula e printa media
AVG_V2_ini = AVG_V2_ini/count_V2_ini
print(AVG_V2_ini)

first=True 
dischargeReached=False
initalTime=0
endTime=0
for num_line in range(2,len(lines)):
    t=float(lines[num_line].split(",")[0])
    ...
    if(v2>AVG_V2_ini and first):
        initalTime=t
        first=False
    if(v2<AVG_V2_ini and not(dischargeReached) and t>time_v2Max):
        endTime=t
        dischargeReached=True

print(endTime)
print(initalTime)
print(endTime-initalTime)
...
for num_line in range(2,len(lines)):
    ...
    v1=float(lines[num_line].split(",")[1])
    v2=float(lines[num_line].split(",")[2])
    cap=t/(resistence*(np.log(1-(v1/Vs))))
    print(cap)
    C.append(t/(resistence*np.log(1-(v1/Vs))))
